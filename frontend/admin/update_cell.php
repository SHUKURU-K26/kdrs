...
<body>
    ...
    <div class="sections">
        ...
        <div class="containers">
            ...
            ?>
              <form action="" method="POST">
                     <div class="new-cell-form">
                        <p>UPDATE <?php echo $row["cell_name"]?> cell</p>
                        <input type="text" name="cell" id="cell" value="<?php echo $row['cell_name']?>" required>
            ...
            ?>
                ...
                $cell_id=$_GET["cell_id"];
                if (isset($_POST["save"])) {
                    $cell=filter_var($_POST['cell'], FILTER_SANITIZE_SPECIAL_CHARS);
                    $sql="UPDATE cells SET cell_name='$cell' WHERE cell_id='$cell_id'";
                    $query=$conn->query($sql);
                    if ($query) {
                      echo"<p id='success-message'>Cell Updated Successfully</p>";
                      echo"
                       <script>
                         setTimeout(()=>{
  ...
  <script>
      let sentence="MAKE SOME CHANGES ON A CELL";
